...
@section('content')
    ...
    <div class="container">
        ...
        <div class="row mb-3">
            <a href="{{route("asistencia.creaPersona")}}"> Show Modal</a>
            <div class="col-md-3 ">
                <div class="card">
                        ...
                        <form class="" action="{{ route('asistencia.buscar') }}" method="GET">
                            ...
                            <div class="mb-3 d-flex align-items-center ">
                                ...
                                <input class="me-2 form-control" type="text" class="form-control" id="cedula"
                                    name="cedula" value="{{ isset($asistente) ? $asistente->id : '' }}" required>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="mb-3 col-md-2 ">
                                <select name="control" id="id_control" class="form-control  " required>
                                    ...
                                    @if ($asignacion)
                                        <option value="{{ $asignacion->control_id }}" selected>
                                            {{ $asignacion->control_id }} </option>
                                    @else
                            ...
                            <div class="col-md-4">
                                <button type="submit" class="btn btn-primary" @disabled($prediosAvailable->isEmpty())>
                                    @if ($asignacion)
                                        Agregar
                                    @else
                                        Asignar
                                ...
                                @isset($prediosAvailable)
                                    @forelse ($prediosAvailable as $predio)
                                        <tr>
                                            ...
                                            <td>{{ $predio->persona->nombre }} {{ $predio->persona->apellido }} </td>
                                            <td>{{ $predio->descriptor1 }} {{ $predio->numeral1 }}
                                                {{ $predio->descriptor2 }} {{ $predio->numeral2 }} </td>
                                            <td>{{ $predio->coeficiente }}</td>
                                            <td>
                                                <input class="form-check-input checkbox-item" type="checkbox" name="predios[]"
                                                    data-coeficiente="{{ $predio->coeficiente }}" value="{{ $predio->id }}"
                                                    id="flexCheckDefault">
                                            </td>
                                        </tr>
                                    @empty
            ...
            <div class="col-md-5">
                ...
                <div class="card ">
                    <div class="card-header  d-flex align-items-center justify-content-between">
                        ...
                        <form action="{{ route('asistencia.getPredios') }}" method="get">
                            <div class="d-flex align-items-baseline ">
                                ...
                                <button type="submit" class="btn ms-1 btn-primary">
                                    <i class='bx bxs-right-arrow-circle bx-w'></i>
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>Nombre</th>
                                    <th>Cedula</th>
                                    <th><a class="btn p-0" href="{{ route('asistencia.dropAllPoderdante') }}"><i
                                                class='bx bxs-message-square-x bx-b'></i></a></th>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table class="table table-hover table-striped">
                            ...
                            <tbody>
                                @isset($asistente)
                                    @forelse ($asistente->asignaciones as $asignacion)
                                    <form action="{{route("asistencia.changeAsignacion")}}" method="get">
                                        <input type="text" name="id_asignacion" value="{{$asignacion->id}}" hidden>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="3">No hay asignaciones</td>
                                        </tr>
                                    @endforelse
                                @else
                                    <tr>
    ...
    <script>
        $(document).ready(function() {
            ...
            function sumarCoeficiente() {
                var coefActual=@json($asignacion ? $asignacion->sum_coef : 0);

...
class Candidatos extends Component
{
    ...
    public function mount()
    {
        if(cache('asamblea')&& ((cache('asamblea')['h_inicio'] || cache('asamblea')['h_cierre']))){
            return redirect()->route('home.elecciones-v2')->with('warning', 'Las elecciones ya se han abierto');
        }

        $this->torres = Torre::all();
        if ($this->torres->isEmpty()) {
            return redirect()->route('elecciones-v2.programar')->with('error', 'No se ha determinado el n̼mero de delegados para cada torre');
        }
    ...
    {
        return view('views.elecciones-v2.candidatos');
    }
    ...
    public function cleanData($cedula)
    {
        ...
        if ($cedula) {
            $this->reset(['cedula']);
            return redirect()->route('elecciones-v2.candidatos');
        }
    }
    ...
    public function storeCandidatos()
    {
        ...
        return redirect()->route('elecciones-v2.candidatos')->with('success', 'Candidatos guardados con exito');
    }
    ...
    public function detachCandidato($cc)
    {
        ...
        return redirect()->route('elecciones-v2.candidatos')->with('success', 'Canditato eliminado correctamente');
    }

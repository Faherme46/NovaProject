...
class Registro extends Component
{
    ...
    public function mount()
    {
        ...
        if(cache('asamblea') && (!cache('asamblea')['h_inicio'] || cache('asamblea')['h_cierre'])){
            return redirect()->route('home.elecciones-v2')->with('warning', 'Las elecciones estan cerradas');
        }
        $this->tipoId = 'CC';
    }
    ...
    {
        return view('views.elecciones-v2.registro');
    }
    ...
    public function cleanData($cedula)
    {
        ...
        if ($cedula) {
            $this->reset(['cedula']);
            return redirect()->route('elecciones-v2.registrar');
        }
    }
    ...
    public function registrar()
    {
        ...
        if ($terminal) {
            return redirect()->route('elecciones-v2.registrar')->with('terminal', $terminal);
        } else {
            return session()->flash('warning', 'No hay terminales libres, espere a que se libere');
        }
    }

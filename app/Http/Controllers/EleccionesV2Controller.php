...
class EleccionesV2Controller extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($qe->errorInfo[1] == 1062) { // 1062 es el cÃ³digo de error para duplicados
        ...
        return redirect()->route('elecciones-v2.programar')->with('success', 'Asamblea creada con Ã©xito, documentos importados.');
    }
    ...
    public function importPredios(String $file)
    {
        try {
            ...
            if (!file_exists($externalFilePathPersonas)) {
                throw new FileNotFoundException("El archivo no se encontrÃ³ en la ruta: {$externalFilePathPersonas}");
            }
            if (!file_exists($externalFilePathPredios)) {
                throw new FileNotFoundException("El archivo no se encontrÃ³ en la ruta: {$externalFilePathPredios}");
            }
            ...
            // Manejar excepciones especÃ­ficas de validaciÃ³n de Excel
            $failures = $e->failures();
            return redirect()->route('elecciones-v2.programar')->withErrors($failures[0]);
            //Excepcion por archivo inexistente
        } catch (FileNotFoundException $e) {
            return redirect()->route('elecciones-v2.programar')->withErrors($e->getMessage());
        } catch (Exception $e) {
    ...
    public function importCandidatos()
    {
        try {
            ...
            if (file_exists($externalFilePathPersonas)) {
                ...
                return redirect()->route('elecciones-v2.candidatos')->with('success', 'Candidatos importados Correctamente');
            } else {
                return redirect()->route('elecciones-v2.candidatos')->with('warning', 'No se importaron candidatos, el archivo personas.xlsx no fue encontrado');
            }
        } catch (Throwable $th) {
            return redirect()->route('elecciones-v2.candidatos')->with('error', 'Error importando candidatos: ' . $th->getMessage());
        }
    }
    ...
    public function generarGraficas()
    {
        ...
        foreach ($torres as $torre) {
            ...
            try {
                ...
                return redirect()->route('elecciones-v2.resultados')->with('error', $th->getMessage());
            }
        }
        return redirect()->route('elecciones-v2.resultados')->with('success', 'GrÃ¡ficas generadas con exito');
    }
    ...
    public function updateElecciones(Request $request)
    {
        $messages = [
            ...
            'fecha.date' => 'El campo :attribute debe ser una fecha vÃ¡lida.',
        ...
        if ($asamblea) {
            ...
            return back()->with('success', 'Asamblea actualizada con Ã©xito.');
        } else {
